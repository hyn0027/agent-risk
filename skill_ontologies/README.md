...
The `discord`, `gh-issues`, `configure-channel`, `coding-agent`, and `taskflow-inbox-triage` entries in `manifest.json` depend on `openclaw-messaging`. Selecting one automatically loads that named graph once, before its skill graph. Selecting an unrelated skill does not. `configure-channel` and `coding-agent` notifications were corrected to invoke `OpenClawMessageCLI`, as their skill text gives the `openclaw message send` shell command; Discord and GitHub notifications explicitly use the agent-facing `message` tool. The loader shows each operation's shell mediation and messaging-route mediation separately.
